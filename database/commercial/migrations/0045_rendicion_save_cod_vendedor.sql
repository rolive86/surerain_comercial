-- rendicion_save: sales_rep cannot set cod_vendedor from client payload.
-- Session vendor is derived from JWT sales_rep_id (fallback: app_user_links).
-- admin / operations / sales_manager may still pass cod_vendedor explicitly.
...
begin
  ...
  v_payload_vend := nullif(btrim(coalesce(p->>'cod_vendedor', '')), '');
  ...
  if v_rep_id is null then
    select l.sales_rep_id into v_rep_id
  ...
  -- sales_rep: always session vendor (ignore client payload).
  -- staff roles: optional override for another vendor.
  if v_role = 'sales_rep' then
    null; -- keep v_cod_vend from session only
  elsif v_payload_vend is not null then
    v_cod_vend := v_payload_vend;
  end if;
